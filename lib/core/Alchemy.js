(function () {
   ...
   /**
    ...
    var alchemy = function (potionName) {
        ...
        if (!potion) {
            potion = alchemy.brew(alchemy.formula.get(potionName));
        }
        return potion;
    };
    ...
    alchemy.path = (function () {
        ...
        function getCorePaths(path) {
            return {
                Ingredient: path + '/Ingredient',
                MateriaPrima: path + '/MateriaPrima',
                Oculus: path + '/Oculus'
            };
        }
        var core = '../../lib/core';
        var pathMap = getCorePaths(core);
        ...
        return {
             ...
             *      NOTICE: The key <code>core</code> is reserved for the packageless
             *      core modules!
             *
             * @memberOf alchemy.path
             */
            set: function (cfg) {
                if (cfg.core) {
                    cfg = alchemy.mix(cfg, getCorePaths(cfg.core));
                    delete cfg.core;
                }
