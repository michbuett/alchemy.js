...
(function () {
    ...
    (function () {
         ...
         * object method (see {@link alchemy.override}
         *
         ...
         */
        alchemy.hocuspocus = function (fn) {
            if (!fn.name) {
                fn.hocuspocus = true;
            }
            return fn;
         ...
         *
         * @param {Object} obj Required. The obj to be extended
         * @param {String} prop Required. The property name
         ...
         *      _prop)
         * @param {Boolean|String} getterName Optional. The name of the getter function; If omitted
         *      the name will be determined by the property (e.g. "id" -> "getId", "name" -> "getName", ...)
         *      If set to <code>false</code> this method will create an anonymous getter method that has
         *      no further references
         *
         * @example
         * alchemy.defineGetter(obj, 'id', myGetter); // adds obj.getId = myGetter to obj
         * alchemy.defineGetter(obj, 'valid', myGetter, "_isValid"); // adds obj._isValid = myGetter to obj
         */
        alchemy.defineGetter = function (obj, prop, getter, getterName) {
            var privateProp = '_' + prop;

            if (getterName === false) {
                // anonym getter method (cannot be overridden)
                ...
                getterName = getterName || 'get' + ucFirst(prop);
         ...
         *      _prop)
         * @param {Boolean|String} setterName Optional. The name of the setter function; If omitted
         *      the name will be determined by the property (e.g. "id" -> "setId", "name" -> "setName", ...)
         *      If set to <code>false</code> this method will create an anonymous setter method that has
         *      no further references
         *
         * @example
         * alchemy.defineSetter(obj, 'id', mySetter); // adds obj.setId = mySetter to obj
         */
        alchemy.defineSetter = function (obj, prop, setter, setterName) {
            var privateProp = '_' + prop;

            if (setterName === false) {
                // anonymous setter
                setter = setter || function (newVal) {
                ...
                // named setter method (determine method name from property if parameter was omitted)
                setterName = setterName || 'set' + ucFirst(prop);
         ...
         * @param {Object} [typeDef.overrides] A set of properties and methods the new prototype should get
         *
         * @return Object
         *      the new prototype
         */
        alchemy.brew = function (typeDef) {
            ...
            if (attributes) {
                alchemy.each(attributes, function (cfg, prop) {
                    var fn;
                    if (cfg.get) {
                        if (alchemy.isString(cfg.get)) {
                            // name is given -> try to function from overrides
                            alchemy.defineGetter(NewType, prop, overrides && overrides[cfg.get], cfg.get);
                        } else if (alchemy.isFunction(cfg.get)) {
                            // function is given -> create an anonymous getter
                            alchemy.defineGetter(NewType, prop, cfg.get, false);
                        }
                    }
                    if (cfg.set) {
                        if (alchemy.isString(cfg.set)) {
                            // name is given -> try to function from overrides
                            alchemy.defineSetter(NewType, prop, overrides && overrides[cfg.set], cfg.set);
                        } else if (alchemy.isFunction(cfg.set)) {
                            // function is given -> create an anonymous setter
                            alchemy.defineSetter(NewType, prop, cfg.set, false);
                        }
                    }
                });
            }
