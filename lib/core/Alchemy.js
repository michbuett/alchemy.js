...
(function () {
    ...
    function brewV2(typeDef) {
        ...
        var potion = alchemy.isFunction(overrides) ? overrides.apply(null, dependencies) : {};
        var meta = [];
        ...
        alchemy.each(meta, function (metaAttr) {
            alchemy.meta(potion, metaAttr[0], metaAttr[1]);
        });

        if (typeof potion.brew !== 'function') {
            potion.brew = defaultFactoryFn;
        }

        if (potion.constructor === Object.prototype.constructor) {
            var Ctor = function (cfg) {
                alchemy.override(this, cfg);
                this.id = this.id || alchemy.id();
            };

            Ctor.prototype = potion;
            potion.constructor = Ctor;
        }
    ...
    alchemy.formula = (function () {
        ...
        function filterCandidates(candidates) {
            ...
            for (var i = 0; i < candidates.length; i++) {
                var c = candidates[i];
                if (alchemy.isString(c) && !resolved[c] && dependencies.indexOf(c) < 0) {
                    dependencies.push(c);
                }
            }
            return dependencies;
        ...
        return {
             ...
             *      The formula to add
             */
            add: function (formula, overrides) {
                ...
                resolved[formula.name] = true;
            },

            /**
             * New experimental API to define potions
             */
