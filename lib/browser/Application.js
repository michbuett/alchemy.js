            ...
            loop: (function () {
                ...
                return function (tick) {
                    if (this.runs) {
                        ...
                        var diff = (tick - this.lastTick) || 1;
                        var fps = (this.frames * 19 + 1000 / diff) / 20;

                        this.lastTick = tick;
                        this.frames = fps;

                        window.requestAnimationFrame(this.boundLoopFn);

                        if (!this.paused) {
                            // update the parameter set for the current iteration
                            loopParams.frame = frame;
                            loopParams.now = Math.round(this.startTime + tick);
