module.exports = function (alchemy) {
    ...
    alchemy.formula.add({
         ...
         * @name RenderContext
         */
        function RenderContext(id, state, props, children, placeholder) {
            this.entityId = id;
            this.state = state;
            this.props = props;
        ...
        return {
            ...
            update: function () {
                ...
                alchemy.each(renderConfigs, this.updateEntity, this, [placeholder, rootNodes]);
            ...
            updateEntity: function (renderCfg, index, placeholder, rootNodes) {
                ...
                var delegatedEvents = this.entities.getComponent(renderCfg.id, 'delegatedEvents');
                if (delegatedEvents && delegatedEvents.current) {
                    var el = renderCfg.current.properties;
                    alchemy.each(delegatedEvents.current.val(), function (cfg) {
                        this.delegator.delegateKey(cfg.event, cfg.delegate, el);
                    }, this);

                    delegatedEvents.last = delegatedEvents.current;
                }
