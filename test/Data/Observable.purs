...
import Alchemy.Data.Increment.Value (fold, fold')
import Alchemy.Data.Incremental (runPatch)
...
tests =
  describe "Alchemy.Data.Observable" do
     ...
     it "allows to observe patching nested data structures" do
        ...
        actual <- liftEffect do
          ref <- Ref.new []
          { ov, sender } <- create $ fold runPatch v
          _ <- subscribe (increments ov) (collect ref)
          sender p1
          sender p2
          Ref.read ref
